...
/*
    ...
    if (document.getElementById) {
        ...
        alert(box.tagName); //DIV 获取这个元素节点的标签名
        alert(box.innerHTML); //测试DIV 获取这个元素节点里的文本(包含html标签)，纯文本不能包含标签
        innerHTML获取的是这个元素的文本内容，而不是文本节点
        alert(box.id); //box, 获取这个元素的id属性的值，注意不是属性节点。
        alert(box.title); //标题, 获取title属性的值
        alert(box.style); //[object CSSStyleDeclaration], 获取style属性对象
        alert(box.style.color); //red, 获取style属性对象中color属性的值
        alert(box.class); //undefined, 貌似是保留字
        alert(box.className); //pox, 获取class属性的值
        以上是HTML属性的直接调用，当然后面还有几种方式可以调用
        alert(box.bbb); //undefined, 自定义属性，直接获取，非IE不支持
