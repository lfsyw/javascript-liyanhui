/*
    ...
    alert(box);
    alert(box.nodeName); //DIV ,获取元素节点的标签名，和tagName等价
    alert(box.nodeType); //1 获取元素节点类型，1表示元素
    alert(box.nodeValue); //null,获取元素节点内容，元素节点本身是没有内容的
    ...
    alert(box.childNodes); //[object NodeList]，获取集合，返回当前元素节点所有的子节点列表
    alert(box.childNodes.length); //3 ，3个子节点
    ...
    alert(box.childNodes[0].nodeType); //3表示本文节点
    alert(box.childNodes[0].nodeValue); //测试Div， 获取文本节点内容
    alert(box.childNodes[0].innerHtml); //undefined ,无效，当前的文本，怎么可能找到里面的内容呢？
